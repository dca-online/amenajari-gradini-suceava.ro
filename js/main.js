            ...
            .map(review => `
                <div class="testimonial-item bg-white rounded p-4 p-sm-5">
                    ...
                    <div class="review-footer">
                        <h5 class="fw-bold mb-2">${review.name}</h5>
...
function updateCityDisplay() {
    const selectedCity = Array.from(document.querySelectorAll('input[name="city"]:checked'))
...
document.getElementById('closeReviewCityMenu')?.addEventListener('click', function(e) {
    e.stopPropagation();
    const selectedCity = Array.from(document.querySelectorAll('input[name="reviewCity"]:checked'))
        .map(radio => radio.value)[0];
...
function updateReviewServiceDisplay() {
    const selectedServices = Array.from(document.querySelectorAll('#reviewServiceMenu input[type="checkbox"]:checked'))
        .map(checkbox => checkbox.value);
