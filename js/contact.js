document.addEventListener("DOMContentLoaded", function() {
	...
	if (contactForm) {
		contactForm.addEventListener("submit", async function(e) {
			...
			if (!subject) {
				errorMessage += "• Vă rugăm să completați subiectul\n"
			}
			else if (subject.length < 5) {
				errorMessage += "• Subiectul trebuie să aibă cel puțin 5 caractere\n"
			}
			if (!messageContent) {
				errorMessage += "• Vă rugăm să introduceți un mesaj\n"
			}
...
${messageContent}
		...
		document.querySelectorAll(".contact-form-card input, .contact-form-card textarea").forEach((input) => {
			input.addEventListener("keypress", function(e) {
				if (e.key === "Enter" && !e.shiftKey) {
					e.preventDefault();
					document.querySelector(".contact-form-card form").dispatchEvent(new Event("submit"))
				}
			})
		})
	}
