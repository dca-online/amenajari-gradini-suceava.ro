document.addEventListener("DOMContentLoaded", () => {
    ...
    function displayImages(category) {
        ...
        portfolioItems.innerHTML = ''; // Clear existing items

        const categoryKey = category.replace('.', '');
        console.log(`Selected category: ${categoryKey}`); // Debugging line

        // Define the folder path for images
        const folderPath = `img/imaginiGradini/${categoryKey}/`;
        
        // List of images in the selected category folder
        const images = [
            // Add all image URLs for the selected category here
        ...
        images.forEach(imageUrl => {
            console.log(`Adding image: ${imageUrl}`); // Debugging line
    ...
    // Set default category to "peisagistica" on page load
    displayImages('peisagistica'); // Call displayImages with the default category
});